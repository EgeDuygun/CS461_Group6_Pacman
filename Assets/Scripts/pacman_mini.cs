...
public class pacman_mini_not : MonoBehaviour {
	...
	public (Vector3, Vector3, Vector3, List<Vector3>, float) better_generate_successor(int agent_index, Vector3 action, Vector3 p_pos, Vector3 g1_pos, Vector3 g2_pos, List<Vector3> pellets, List<Vector3> walls, float score) {
        ...
        if (walls.Contains(new Vector3(nextx, nexty, 0)) == false) {
            ...
            if (agent_index == 0) {
				...
				float close_food = closest_food(nextState, main_pellets);				
				//float negatives = (1/(close_ghost + 1)) + 10*(main_pellets.Count);
				//float positives = (100/(close_food + 0.001f));
				float negatives = (1/(close_ghost + 0.00002f)) + 10*(main_pellets.Count);
				float positives = (10/(close_food + 0.1f));
	...
	void Start () {	
		...
		while (true) {
			...
			counter = counter + 1;
			pacman_action = ab_pacmanNextAction(p_pos, g1_pos, g2_pos, pellet_list, wall_list, score);
